...
const deduplicate = (obj, options = {}) => {
  ...
  const processObject = (obj, category = "root", path = "root") => {
    if (Array.isArray(obj)) {
      ...
      // If the object already exists in the database, use the existing ID
      if (idsMap.has(key)) {
      ...
      if (counts[key] > 1 || (alwaysReplaceWithId && obj.id)) {
        ...
        if (!database[currentCategory][key]) {
          ...
          idsMap.set(key, newId); // Map the key to the new ID
          return newId;
        }
      }

      // For unique items, return the object as it is
      return obj;
    }
    return obj;
  };
